...
pub(crate) fn fit_closed_h_potrace_segments(points: &[(f64, f64)]) -> Option<Vec<SvgPathSegment>> {
    ...
    let candidates = [
        h_base_potrace_segments(bounds),
        h_mx_potrace_segments(bounds),
        h_my_potrace_segments(bounds),
        h_r90_potrace_segments(bounds),
        h_r180_potrace_segments(bounds),
        h_r270_potrace_segments(bounds),
    ];
